...
func (r *PostgresHealthRepository) Create(ctx context.Context, record *models.HealthRecord) error {
...
func (r *PostgresHealthRepository) GetByHorseID(ctx context.Context, horseID uint) ([]models.HealthRecord, error) {
...
func (r *PostgresHealthRepository) Update(ctx context.Context, record *models.HealthRecord) error {
	return r.db.WithContext(ctx).Save(record).Error
}

func (r *PostgresHealthRepository) Delete(ctx context.Context, id uint) error {
...
func (r *PostgresBreedingRepository) GetByID(ctx context.Context, id uint) (*models.BreedingRecord, error) {
...
func (r *PostgresPregnancyRepository) InitializePreFoalingChecklist(ctx context.Context, pregnancyID uint) error {
	defaultItems := []models.PreFoalingChecklistItem{
		{HorseID: pregnancyID, Description: "Prepare foaling kit", IsCompleted: false},
		{HorseID: pregnancyID, Description: "Clean and disinfect foaling area", IsCompleted: false},
		{HorseID: pregnancyID, Description: "Check camera system", IsCompleted: false},
		{HorseID: pregnancyID, Description: "Prepare emergency contacts", IsCompleted: false},
		{HorseID: pregnancyID, Description: "Stock up on supplies", IsCompleted: false},
	}
...
func (r *PostgresExpenseRepository) Create(ctx context.Context, expense *models.Expense) (*models.Expense, error) {
	...
	if result.Error != nil {
		return nil, fmt.Errorf("failed to create expense: %w", result.Error)
	}
	return expense, nil
}
...
func (r *PostgresExpenseRepository) Update(ctx context.Context, expense *models.Expense) (*models.Expense, error) {
	...
	if result.Error != nil {
		return nil, fmt.Errorf("failed to update expense: %w", result.Error)
	}
	return expense, nil
}

func (r *PostgresExpenseRepository) GetByID(ctx context.Context, expenseID uint) (*models.Expense, error) {
	var expense models.Expense
	result := r.db.WithContext(ctx).First(&expense, expenseID)
	if result.Error != nil {
...
func (r *PostgresExpenseRepository) Delete(ctx context.Context, expenseID uint) error {
	result := r.db.WithContext(ctx).Delete(&models.Expense{}, expenseID)
	if result.Error != nil {
...
func (r *PostgresRecurringExpenseRepository) Create(ctx context.Context, recurringExpense *models.RecurringExpense) (*models.RecurringExpense, error) {
	result := r.db.WithContext(ctx).Create(recurringExpense)
	if result.Error != nil {
		return nil, fmt.Errorf("failed to create recurring expense: %w", result.Error)
	}
	return recurringExpense, nil
}

func (r *PostgresRecurringExpenseRepository) Update(ctx context.Context, recurringExpense *models.RecurringExpense) (*models.RecurringExpense, error) {
	result := r.db.WithContext(ctx).Save(recurringExpense)
	if result.Error != nil {
		return nil, fmt.Errorf("failed to update recurring expense: %w", result.Error)
	}
	return recurringExpense, nil
}

func (r *PostgresRecurringExpenseRepository) GetByID(ctx context.Context, recurringExpenseID uint) (*models.RecurringExpense, error) {
	var expense models.RecurringExpense
	result := r.db.WithContext(ctx).Model(&models.RecurringExpense{}).First(&expense, recurringExpenseID)
	if result.Error != nil {
		return nil, result.Error
	}
	return &expense, nil
...
func (r *PostgresRecurringExpenseRepository) Delete(ctx context.Context, recurringExpenseID uint) error {
	result := r.db.WithContext(ctx).Delete(&models.RecurringExpense{}, recurringExpenseID)
	if result.Error != nil {
		return fmt.Errorf("failed to delete recurring expense: %w", result.Error)
	}
	if result.RowsAffected == 0 {
		return gorm.ErrRecordNotFound
	}
...
func (r *PostgresRecurringExpenseRepository) GetByUserID(ctx context.Context, userID string) ([]models.RecurringExpense, error) {
	var expenses []models.RecurringExpense
	result := r.db.WithContext(ctx).Where("user_id = ?", userID).Find(&expenses)
	if result.Error != nil {
		return nil, fmt.Errorf("failed to get recurring expenses by user ID: %w", result.Error)
	}
	return expenses, nil
}

func (r *PostgresRecurringExpenseRepository) GetDueRecurringExpenses(ctx context.Context) ([]models.RecurringExpense, error) {
	var expenses []models.RecurringExpense
	now := time.Now()
	result := r.db.WithContext(ctx).
		Where("next_due_date <= ? AND (end_date IS NULL OR end_date > ?)", now, now).
		Find(&expenses)
	if result.Error != nil {
		return nil, fmt.Errorf("failed to get due recurring expenses: %w", result.Error)
	}
	return expenses, nil
}