...
describe('HorseList', () => {
	...
	it('handles network errors gracefully', async () => {
		...
		renderWithProviders(<HorseList />);
		expect(
			await screen.findByText(/failed to load horses/i)
		).toBeInTheDocument();
		expect(
	...
	it('shows pregnancy status for pregnant mares', async () => {
		...
		renderWithProviders(<HorseList />);
		expect(
			await screen.findByText(/180 days to foaling/i)
		).toBeInTheDocument();
	...
	it('shows different pregnancy stages with correct colors', async () => {
		...
		expect(badge).toHaveStyle({
			backgroundColor: expect.stringContaining('teal'),
		});
	...
	it('handles pregnancy data loading errors', async () => {
		...
		renderWithProviders(<HorseList />);
		expect(
			await screen.findByText(/failed to fetch pregnancy status/i)
		).toBeInTheDocument();
	});
